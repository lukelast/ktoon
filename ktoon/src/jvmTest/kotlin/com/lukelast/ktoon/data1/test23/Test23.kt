 ...
 * underlying map, so the document is an ordinary object (§8) that the CLI and ktoon both encode
 * the same way — the point of the golden is the decode leg, where the top-level serializer's own
 * descriptor is a `CLASS`, not a `MAP`, and the root decoder still has to hand the map
 * deserializer a map-shaped decoder.
 */
